'use strict';
import React, { useState, useEffect }  from 'react';
import {AuthFormItem} from './AuthFormsItem/AuthFormsItem';
import axios from 'axios';


export const Auth = () => {
    let [isAuth, setAuth] = useState(false);
    
    const validateEmail = (data:string) => {
        const reg = new RegExp(/(?:[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])/);
        return (reg.test(data)) ? true : false;
    }
    
    const validatePass = (data:string) => {
        return (data.trim() && data.length > 0 && data.length < 50 && data.search(/[А-Яа-яЁ-ё]/) === -1) ? true : false;
    }

    // useEffect(() => {Boolean(localStorage.getItem('isAuth')) ? setAuth(isAuth = true) : '';})
    // console.log('this isAuth =' + isAuth);
    return (
        <AuthFormItem validateEmail={validateEmail} validatePassword={validatePass}/>
    )
}
